...
console.log('╔════════════════════════════════════════════════════╗');
console.log('║   清如 ClearSpring V2 - API 路由验证                ║');
console.log('╚════════════════════════════════════════════════════╝\n');
...
routeFiles.forEach(file => {
  ...
  const status = exists ? '✅' : '❌';
  console.log(`${status} ${file}: ${exists ? '存在' : '不存在'}`);
...
routeDefinitions.forEach((route, index) => {
  ...
  console.log(`${status} ${route.file}: ${route.description} - ${isDefined ? '已定义' : '未定义'}`);
...
docs.forEach(doc => {
  ...
  const status = exists ? '✅' : '❌';
  console.log(`${status} ${path.basename(doc)}: ${exists ? '已生成' : '未生成'}`);
...
// 总结
if (allPassed) {
  console.log('✅ 所有检查通过！5 个 API 接口已全部实现并正确注册。\n');
  console.log('下一步:');
  console.log('1. 启动服务器: npm start');
  console.log('2. 设置 Token: export ADMIN_TOKEN="your_token"');
  console.log('3. 运行测试: node tests/missing-api.test.js\n');
  process.exit(0);
} else {
  console.log('❌ 部分检查未通过，请检查上述输出。\n');
  process.exit(1);
}
